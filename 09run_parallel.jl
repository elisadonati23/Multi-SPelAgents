...
# Initialize models
models = [
    model_initialize_parallel(10000.0, 0.0, 0.0, 0.0, 1.7e14, 1.0, 0.2 , 0.945, 15.0, 0.9995, 1.06,  0.86, 0.69,0.62,0.48) 
]
...
plot_population_timeseries(results[1][1], missing, true)
...
plot_param_timeseries(results[1][2], [:f])
#mean adult and juvenile length
plot_means_with_std(results[1][2], [:meanAdL, :meanJuvL], [:sdAdL, :sdJuvL])

# Total annual fished biomass in tonnes 
plot_annual_param_timeseries(results[1][2],[:fishedW], true, :sum, "Annual sum of fished weight")
