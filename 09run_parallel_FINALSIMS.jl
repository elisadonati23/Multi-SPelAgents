...
model_initialize_parallel(1000.0, 0.0, 0.0, Mf0_run, Mf1_run, Mf2_run, Mf3_run, Mf4_run, 1.7e14, 1.0, Xmax50percent, 0.945, Temp_run, 0.9998,	1.08,	0.86,	0.69,	0.62,	0.48)
]
...
for (i, model) in enumerate(models)
    ...
    CSV.write("diagnostic_+FOOD_agentFINALSIMS_$((i)).csv", results[i][1])
    CSV.write("diagnostic_+FOOD_modelFINALSIMS_$((i)).csv", results[i][2])
    #save plots
    Plots.savefig(p1, "diagnostic_FINALSIMS_p1_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(p2, "diagnostic_FINALSIMS_p2_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(pop, "diagnostic_FINALSIMSpop_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(deaths, "diagnostic_FINALSIMSdeaths_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(biom, "diagnostic_FINALSIMSbiom_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(fr, "diagnostic_FINALSIMSfr_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(lengths, "diagnostic_FINALSIMSlengths_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(fished, "diagnostic_FINALSIMSfished_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
    Plots.savefig(summerbiom, "diagnostic_FINALSIMSsummerbiom_$(Dates.format(today(), "yyyy-mm-dd"))_$((i)).png")
end
